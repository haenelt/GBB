def update_mesh(vtx, vtx_shift, source_ind, nn_ind, l_rate=0.9):
    ...
    neighboorhood and specified learning rate. If the neighborhood only contains the source vertex,
    no distance weighting is computed. The learning rate should be a value between 0 and 1. Vertex 
    coordinates are in ras space.
    Inputs:
