def get_direction_euclidean(pt_source, vtx_ref, diff_dir=2, diff_threshold=0.05):
    """
    ...
    points by taking the difference source point minus reference point. The sign of the distance is 
    used to infer the direction between white and pial surface.
    Inputs:
        *pt_source: source vertex point (white surface).
    ...
    rx_temp = ( vtx_ref[:,0] - pt_source[0] ) ** 2
    ry_temp = ( vtx_ref[:,1] - pt_source[1] ) ** 2
    rz_temp = ( vtx_ref[:,2] - pt_source[2] ) ** 2
    r_temp = np.sqrt(rx_temp + ry_temp + rz_temp)
       
    # reference index number closest to current source vertex
    ind_min = np.argmin(r_temp)
       
    # distance between points along specified axis
    r_dist = pt_source[diff_dir] - vtx_ref[ind_min,diff_dir]
