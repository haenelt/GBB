def plot_normal_direction(input_surf, line_dir=2, diff_threshold=0.05):
    """
    ...
    Inputs:
        ...
        *line_dir: axis for distance calculation in ras space.
    ...
    # get directions 
    r_dist, _ = get_normal_direction(vtx_source, fac_source, line_dir, diff_threshold)

    # write output
    header = nb.freesurfer.mghformat.MGHHeader()
    output = nb.freesurfer.mghformat.MGHImage(r_dist, np.eye(4), header)
    nb.save(output, input_surf+"_plot_normal_dir"+str(line_dir)+".mgh")